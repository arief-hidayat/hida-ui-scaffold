...
(function($, Backbone, _, moment, App){
    ...
    App.view.SearchTableFormPage = App.View.extend({
        el : '#content-section',
        tableEl : '#list-section', searchEl : "#search-section", detailEl : '#detail-section', messageEl : ".message-container",
...
//                App.logDebug("enter loadForm from url" + url);
        ...
        buildTable : function(publishReload) {
            if(this.tableView == undefined) {
                this.tableView = new App.view.TableTabRegion( {el: this.tableEl, key: this.key,
                    pubSub: this.tablePubSub, customUrl : this.tableConfig} ); // TODO: change to normal Table.
            }
            else this.tableView.reloadTable();
...
//            this.form = new App.view.EditableForm(options);
        ...
        setupInitView : function(opt) {
            this.$message = this.$(this.messageEl);
            //TODO: build search form.

            this.buildTable();
            this.initTableActionListeners(opt);
//            if(opt.initialForm && opt.initialForm.id) {
//                this.buildForm(opt.initialForm);
//            }
